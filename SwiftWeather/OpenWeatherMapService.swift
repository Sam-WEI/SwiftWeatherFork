...
struct OpenWeatherMapService: WeatherServiceProtocol {
  ...
  func retrieveWeatherInfo(location: CLLocation, completionHandler: @escaping WeatherCompletionHandler) {
    let sessionConfig = URLSessionConfiguration.default
    let session = URLSession(configuration: sessionConfig)

    guard let url = generateRequestURL(location: location) else {
    ...
    let task = session.dataTask(with: request,
      ...
      weatherBuilder.forecasts = self.getFirstFourForecasts(json: json)
  ...
  private func generateRequestURL(location: CLLocation) -> URL? {
    ...
    let appId = (parameters!["OWMAccessToken"]! as AnyObject).description
